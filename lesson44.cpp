...
bool init() {
    ...
    if(SDL_Init(SDL_INIT_VIDEO) < 0) {
        ...
        w = SDL_CreateWindow("Movimiento independiente de los fps", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if(w == NULL) {
            cout << SDL_GetError() << endl;
            success = false;
        } else {
...
bool loadMedia() {
    ...
    if(!dotTexture.loadFromFile("assets/lesson44/dot.bmp")) {
        success = false;
    }
    ...
    if(init()) {
        if(loadMedia()) {
            ...
            while(!quit) {
                ...
                dot.move((finalTime - inicio)/1000.f);
